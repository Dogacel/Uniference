...
from fairscale.nn.model_parallel.initialize import get_model_parallel_world_size, get_model_parallel_rank
from fairscale.nn.model_parallel.mappings import (
    copy_to_model_parallel_region as ctmpr,
    gather_from_model_parallel_region as gfmpr,
    reduce_from_model_parallel_region as rfmpr,
    scatter_to_model_parallel_region as stmpr,
...
class RowParallelLinearSim(nn.Module):
    ...
    def __init__(
        ...
        super(RowParallelLinearSim, self).__init__()
        self.me = me
        ...
        if self.me.world.backend == "pytorch":
            world_size = get_model_parallel_world_size()
            rank = get_model_parallel_rank()
        else:
            world_size = len(me.tp_chan().subscribers)
            rank = me.tp_chan().subscribers.index(me)
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:  # type:ignore
        ...
        if self.input_is_parallel:
            ...
            input_parallel = scatter_to_model_parallel_region(input_, self.me)
        # Matrix multiply.
        output_parallel = F.linear(input_parallel, self.weight)
        # All-reduce across all the partitions.
        output_ = reduce_from_model_parallel_region(output_parallel, self.me)
...
class ColumnParallelLinearSim(torch.nn.Module):
    ...
    def __init__(
        ...
        super(ColumnParallelLinearSim, self).__init__()
        self.me = me
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:  # type: ignore
        # Set up backprop all-reduce.
        input_parallel = ctmpr(input_)
        # Matrix multiply.
        output_parallel = F.linear(input_parallel, self.weight, self.bias)
        if self.gather_output:
            # All-gather across the partitions.
            output = gather_from_model_parallel_region(output_parallel, self.me)
        else:
            output = output_parallel
        return output


class VocabParallelEmbeddingSim(torch.nn.Module):
    ...
    def __init__(
        ...
        super(VocabParallelEmbeddingSim, self).__init__()
        self.me = me
        ...
        _initialize_affine_weight(
            self.weight, self.num_embeddings, self.embedding_dim, self.num_embeddings_per_partition, 0, init_method, me
        )
...
def reduce_from_model_parallel_region(input_: torch.Tensor, me: Device) -> torch.Tensor:
    ...
    result = me.tp_chan().all_reduce(me, input_, "reduce_from_model_parallel_region")
    return result
...
def gather_from_model_parallel_region(input_: torch.Tensor, me: Device) -> torch.Tensor:
    ...
    result = me.world.chan(f"tp_{me.tp_group}").all_gather(me, input_, "gather_from_model_parallel_region")
